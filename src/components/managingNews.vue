...
export default {
  ...
  methods: {
    ...
    getNewsList(page, size) {
      this.axios
        .get(
          `${process.env.VUE_APP_URL}/api/news/list?page=${page}&size=${size}`,
          {
            headers: {
        ...
        .then((res) => {
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            ...
            if (this.tableData[i].coverImg) {
              this.tableData[
                i
              ].coverImg = `${process.env.VUE_APP_URL}/api/news/picture/${this.tableData[i].coverImg}`;
            }
          }
        });
    },
    ...
    handleDelete(index, row) {
      this.axios
        .delete(
          ` ${process.env.VUE_APP_URL}/api/news/${row.newsId}`,
    ...
    // 浏览
    goView(newsId) {
      window.open(`/newsInformation/newsPage?newsId=${newsId}`);
    },
  },
};
</script>
